...
export function ComparisonPage() {
  ...
  const columns: ColDef[] = useMemo(() => {
    ...
    return (
      compareMut.data.entityType === "bd-activity" ? BD_COLS : CAMPAIGN_COLS
    ) as ColDef[];
  }, [compareMut.data]);
  ...
  const bestWorst = useMemo(() => {
    ...
    for (const col of columns) {
      ...
      const vals = sorted
        .map((r) => (r as unknown as Record<string, unknown>)[col.key])
        ...
        <CardContent>
          {itemsLoading ? (
            ...
            <p className="text-sm text-muted-foreground py-6 text-center">
              No{" "}
              {entityType === "bd-activity"
                ? "BD activities"
                : `${entityType}s`}{" "}
              found. Add data first.
            </p>
          ) : (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {item.spend != null && (
                          <span> &middot; {fmt(item.spend)}</span>
                        )}
                        {item.cost != null && (
                          <span> &middot; {fmt(item.cost)}</span>
                        )}
                      </p>
                    </div>
                  </button>
