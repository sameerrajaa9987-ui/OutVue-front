    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Test "what-if" budget scenarios to project impact on leads, CPL, and
          ROI.
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                onClick={addRow}
                disabled={adjustments.length >= CHANNELS.length}
                className="gap-1.5"
              >
              ...
              <Button
                ...
                disabled={
                  ...
                  adjustments.every((a) => a.changePercent === 0)
                }
                className="gap-1.5"
              >
                {scenarioMut.isPending ? (
        ...
        <CardContent>
          <div className="space-y-3">
            {adjustments.map((adj, idx) => (
              <div
                key={idx}
                className="flex items-end gap-3 rounded-lg border border-border bg-muted/20 p-3"
              >
                  ...
                  >
                    {CHANNELS.map((c) => (
                      <option
                        key={c}
                        value={c}
                        disabled={adjustments.some(
                          (a, i) => i !== idx && a.channel === c,
                        )}
                      >
                        {c}
                      </option>
                ...
                <div className="w-[140px] space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateRow(idx, "changePercent", Number(e.target.value))
                    }
                  />
                </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-lg font-bold">
                    {fmt(result.projectedSpend)}
                  </span>
                  <DeltaBadge
                    current={result.currentSpend}
                    projected={result.projectedSpend}
                    inverse
                  />
                </div>
                <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                  {fmt(result.currentSpend)} <ArrowRight className="h-3 w-3" />{" "}
                  {fmt(result.projectedSpend)}
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-lg font-bold">
                    {fmtNum(result.projectedLeads)}
                  </span>
                  <DeltaBadge
                    current={result.currentLeads}
                    projected={result.projectedLeads}
                  />
                </div>
                <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                  {fmtNum(result.currentLeads)}{" "}
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-lg font-bold">
                    {fmt(result.projectedCPL)}
                  </span>
                  <DeltaBadge
                    current={result.currentCPL}
                    projected={result.projectedCPL}
                    inverse
                  />
                </div>
                <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                  {fmt(result.currentCPL)} <ArrowRight className="h-3 w-3" />{" "}
                  {fmt(result.projectedCPL)}
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-lg font-bold">
                    {pct(result.projectedROI)}
                  </span>
                  <DeltaBadge
                    current={result.currentROI}
                    projected={result.projectedROI}
                  />
                </div>
                <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                  {pct(result.currentROI)} <ArrowRight className="h-3 w-3" />{" "}
                  {pct(result.projectedROI)}
                </p>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <p className="text-xl font-bold">
                    {fmt(result.projectedRevenue)}
                  </p>
                  <DeltaBadge
                    current={result.currentRevenue}
                    projected={result.projectedRevenue}
                  />
                </div>
              </CardContent>
            </Card>
                      ...
                      {result.channelDetails.map((d, idx) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right tabular-nums font-medium">
                            {fmt(d.projectedSpend)}
                          </td>
                          <td className="px-4 py-3 text-right tabular-nums">
                            {fmtNum(d.currentLeads)}
                          </td>
                          <td className="px-4 py-3 text-right tabular-nums font-medium">
                            {fmtNum(d.projectedLeads)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
